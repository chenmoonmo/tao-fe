...
function Page() {
  ...
  const { data: weightsData } = useQuery({
    queryKey: ["validator", "weights", address, subnetContractAddress],
    queryFn: async () => {
      const data = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/validatorWeights?validator=${address}&subnet=${subnetContractAddress}`
      ).then((res) => res.json());
    },
  });
  ...
  const [enabledRegister, enableSetWeights] = useMemo(() => {
    ...
    return [+data.StakeAmount > 0 && !data.LegalValidator, false];
  }, [data]);
      ...
      <div>
        <Card size="4" mt="5">
          <div className="grid grid-cols-[1fr,max-content,max-content,max-content] gap-3 w-[500px]">
            ...
            <TextFieldInput id="weight-1" className="!w-24" />

            <label className="col-span-2">mineraddress1</label>
